...
- 오늘의 배차, 달력(월~금 3.5t/5t/기타 + 일요일 미정 보관함) 등 배차 관리 기능
- 재고 관리 (회사 전체 공용 품목별 수량 — 등록/수정/삭제/수량조정은 관리자만, 기사님은 조회만)
- 게시판식 메모 (회사 전체 공유 공지 — 작성/수정/삭제는 관리자만, 기사님은 조회만)
...
- **로그인 상태 30일 유지**: 로그인 세션을 서버 메모리가 아니라 Neon(PostgreSQL)에 저장하므로, Render가 서버를 재시작/슬립시켜도 로그인이 풀리지 않습니다. 앱을 계속 쓰는 한(30일 이내 접속) 로그인이 자동으로 계속 연장되어, 기사님들이 매번 다시 로그인할 필요가 없습니다. 30일 넘게 아예 안 쓰거나 로그아웃 버튼을 직접 누르면 다시 로그인해야 합니다.
...
거래처/배차/재고/메모 데이터는 **Neon(PostgreSQL) 데이터베이스**에 저장됩니다. 서버를 처음 실행하면 필요한 테이블(`clients`, `schedule_days`, `schedule_items`, `inventory_items`, `bulletin_notes`, 로그인 세션을 저장하는 `user_sessions`)이 자동으로 만들어지므로 별도로 테이블을 만들 필요는 없습니다.
