...
거래처/배차 데이터는 **Neon(PostgreSQL) 데이터베이스**에 저장됩니다. 서버를 처음 실행하면 필요한 테이블(`clients`, `schedule_days`, `schedule_items`, 로그인 세션을 저장하는 `user_sessions`)이 자동으로 만들어지므로 별도로 테이블을 만들 필요는 없습니다.
